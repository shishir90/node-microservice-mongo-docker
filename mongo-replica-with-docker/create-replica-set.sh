...
function copyFilesToContainer {
  ...
  docker cp ./users.js $1:/data/admin
}
...
function add_usersdb_test {
  ...
  sleep 1
  docker exec -i mongoNode1 bash -c 'mongo -u $MONGO_USER_ADMIN -p $MONGO_PASS_ADMIN --authenticationDatabase "admin" < /data/admin/users.js'
}
